...
def main():
  ...
  parser.add_argument('--training_epochs', type=int, default=9999,
  ...
  parser.add_argument('--learning_rate_vae', type=float, default=0.0005,
...
def train(args):
  ...
  for epoch in range(training_epochs):
    ...
    for i in range(total_batch):
      ...
      d_loss, g_loss, vae_loss, vae_loss_l2, vae_loss_kl, n_operations = cppnvae.partial_train(batch_images)
      ...
      if (counter+1) % display_step == 0:
        print "Sample:", '%d' % ((i+1)*batch_size), " Epoch:", '%d' % (epoch), \
              ...
              "l2=", "{:.4f}".format(vae_loss_l2), \
              "kl=", "{:.4f}".format(vae_loss_kl), \
              "n_op=", '%d' % (n_operations)
      counter += 1

      assert( vae_loss < 100000000000 ) # make sure it is not NaN or Inf
      assert( g_loss < 100000000000 ) # make sure it is not NaN or Inf
      assert( d_loss < 100000000000 ) # make sure it is not NaN or Inf
