...
class CPPNVAE():
  def __init__(self, batch_size=1, z_dim=32,
                x_dim = 30, y_dim = 30, c_dim = 3, scale = 8.0,
                learning_rate= 0.01, learning_rate_d= 0.001, learning_rate_vae = 0.0001, beta1 = 0.9, net_size_g = 64, net_size_q = 512, net_depth_g = 8,
                keep_prob = 1.0, df_dim = 20, model_name = "cppnvae"):
    """
    ...
    qf_dim              encoder is a convnet.  higher -> more activtions -> better encoder.
  ...
  def create_vae_loss_terms(self):
    ...
    '''
    # use L2 loss instead:
    d = (self.batch_flatten - self.batch_reconstruct_flatten)
    d2 = tf.mul(d, d) * 2.0
    ...
    self.vae_loss_kl = -0.5 * tf.reduce_sum(1 + self.z_log_sigma_sq
                                       ...
                                       - tf.exp(self.z_log_sigma_sq), 1) * 1.0
    self.vae_loss_kl = tf.reduce_mean(self.vae_loss_kl) / self.n_points # average over batch and pixel
    self.vae_loss =  self.vae_loss_l2 + self.vae_loss_kl
  ...
  def coordinates(self, x_dim = 32, y_dim = 32, scale = 8.0):
    n_pixel = x_dim * y_dim
    x_range = scale*(np.arange(x_dim)-(x_dim-1)/2.0)/(x_dim-1)/0.5
    y_range = scale*(np.arange(y_dim)-(y_dim-1)/2.0)/(y_dim-1)/0.5
    #x_mat = np.matmul(np.ones((y_dim, 1)), x_range.reshape((1, x_dim)))
    x_mat = np.ones((y_dim, 1)) * x_range.reshape((1, x_dim))
    #y_mat = np.matmul(y_range.reshape((y_dim, 1)), np.ones((1, x_dim)))
    y_mat = y_range.reshape((y_dim, 1)) * np.ones((1, x_dim))
  ...
  def encoder(self):
    ...
    '''
    h0 = tf.nn.dropout(lrelu(conv2d(self.batch, self.qf_dim, name=self.model_name+'_q_h0_conv')), self.keep_prob)
    h1 = tf.nn.dropout(lrelu(conv2d(h0, self.qf_dim*2, name=self.model_name+'_q_h1_conv')), self.keep_prob)
    h2 = tf.nn.dropout(lrelu(conv2d(h1, self.qf_dim*4, name=self.model_name+'_q_h2_conv')), self.keep_prob)

    z_mean = linear(tf.reshape(h2, [self.batch_size, -1]), self.z_dim, self.model_name+'_q_out_mean')
    z_log_sigma_sq = linear(tf.reshape(h2, [self.batch_size, -1]), self.z_dim, self.model_name+'_q_out_log_sigma_sq')
    '''
  ...
  def discriminator(self, image, reuse=False):
    ...
    '''
    h1 = lrelu(conv2d(h0, self.df_dim*2, name=self.model_name+'_d_h1_conv'))
    h2 = lrelu(conv2d(h1, self.df_dim*4, name=self.model_name+'_d_h2_conv'))
    '''
  ...
  def partial_train(self, batch):
    ...
    for i in range(16):
      counter += 1
      _, vae_loss, vae_loss_l2, vae_loss_kl = self.sess.run((self.vae_opt, self.vae_loss, self.vae_loss_l2, self.vae_loss_kl),
                              feed_dict={self.batch: batch, self.x: self.x_vec, self.y: self.y_vec, self.r: self.r_vec})
    ...
    return d_loss, g_loss, vae_loss, vae_loss_l2, vae_loss_kl, counter
  ...
  def generate(self, z=None, x_dim = 30, y_dim = 30, scale = 8.0):
